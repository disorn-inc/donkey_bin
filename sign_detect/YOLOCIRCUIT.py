...
def intitial():
    ...
    rospy.Subscriber('/sign_bool',Bool,Sign_detect,queue_size=10,queue_size=2**24)
...
def Sign_detect(data, servo_angle, original_angle):
    global count_sign
    # SIGN: LEFT LEFT RIGHT LEFT
    sign_boolean = data
    if sign_boolean=1:
        count_sign+=1
    ...
    if count_sign=1:
        ...
        servo_angle=0
        pub = rospy.Publisher('/allow_yolo', Bool, queue_size=10)
        pub.publish(1)
        if original_angle==50: #mid servo
            count_sign+=1
    elif count_sign=3:
        ...
        #turn left (and go to finish) for ... seconds
        servo_angle=180
        pub = rospy.Publisher('/allow_yolo', Bool, queue_size=10)
        pub.publish(1)
        if original_angle==50: #mid servo
...
def Getcam(data):
    ...
    Sign_detect(cv_image,Servo_motor,Servo_motor)
    
    # sign = rospy.Subscriber('/sign_bool',Bool,queue_size=10)
    # elif ()
